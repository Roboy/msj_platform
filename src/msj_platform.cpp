...
void MSJPlatform::publishStatus(){
    ...
    msg.id = 5;
    msg.pwm_ref.resize(NUMBER_OF_MOTORS);
    msg.position.resize(NUMBER_OF_MOTORS);
    msg.velocity.resize(NUMBER_OF_MOTORS);
    msg.current.resize(NUMBER_OF_MOTORS);
    msg.displacement.resize(NUMBER_OF_MOTORS);
...
//            int32_t vel = MSJ_READ_sensor_angle_velocity(msj_platform_base,i);
            ...
            msg.pwm_ref[i] = pwm;
            msg.velocity[i] = (angle - msg.position[i])/r.cycleTime().toSec();
